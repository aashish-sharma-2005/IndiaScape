...
async function getStatesData(req,res){
    try{
        ...
        const visibleStateIds=visibleStates.map(state=>state._id);
        const places=await Famous.find({state_id:{$in:visibleStateIds}}).populate("state_id","name visible");
        return res.status(200).json({status:true,states:visibleStates,places});
    }catch(error){
        console.log(error);
